# import asyncio
# from random import randint
# from PIL import Image
# import requests
# from dotenv import get_key
# import os
# from time import sleep
# import base64
# import json

# # Set API URL and headers
# API_URL = "https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0"
# # headers = {"Authorization": f"Bearer {get_key('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')}"}
# headers = {"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"}

# # Ensure the Data folder exists
# if not os.path.exists("Data"):
#     os.makedirs("Data")

# def open_images(prompt):
#     folder_path = r"Data"
#     prompt = prompt.replace(" ", "_")
#     files = [f"{prompt}{i}.jpg" for i in range(1, 5)]

#     for jpg_file in files:
#         image_path = os.path.join(folder_path, jpg_file)

#         try:
#             img = Image.open(image_path)
#             print(f"Opening image: {image_path}")
#             img.show()
#             sleep(1)

#         except IOError:
#             print(f"Unable to open {image_path}. Ensure the image file exists and is valid.")

# async def query(payload):
#     try:
#         response = await asyncio.to_thread(requests.post, API_URL, headers=headers, json=payload)
#         response.raise_for_status()  # Raise an error for HTTP failures
#         return response.content
#     except requests.exceptions.RequestException as e:
#         print(f"Error querying API: {e}")
#         return None

# async def generate_images(prompt: str):
#     tasks = []
#     for i in range(4):
#         seed = randint(0, 1000000)
#         payload = {
#             "inputs": f"{prompt}, quality=4k, sharpness=maximum, Ultra High details, high resolution, seed={seed}"
#         }
#         task = asyncio.create_task(query(payload))
#         tasks.append(task)

#     responses = await asyncio.gather(*tasks)

#     for i, response_content in enumerate(responses):
#         if response_content:
#             try:
#                 response_json = json.loads(response_content)
#                 if "images" in response_json:
#                     # Assuming the response contains base64-encoded image data
#                     image_base64 = response_json["images"][0]
#                     image_bytes = base64.b64decode(image_base64)

#                     with open(fr"Data\{prompt.replace(' ', '_')}{i + 1}.jpg", "wb") as f:
#                         f.write(image_bytes)
#                 else:
#                     print(f"Unexpected API response format: {response_json}")
#             except Exception as e:
#                 print(f"Error saving image {i + 1}: {e}")

# def GenerateImages(prompt: str):
#     asyncio.run(generate_images(prompt))
#     open_images(prompt)

# # Main execution loop
# while True:
#     try:
#         with open(r"Frontend\Files\ImageGeneration.data", "r") as f:
#             data = str(f.read())

#         prompt, status = data.split(",")
#         status = status.strip()

#         if status.lower() == "true":
#             print("Generating Images...")
#             GenerateImages(prompt=prompt)

#             with open(r"Frontend\Files\ImageGeneration.data", "w") as f:
#                 f.write("False, False")
#             break
#         else:
#             sleep(1)

#     except :
#         pass




import asyncio
import base64
import json
import os
from time import sleep
from random import randint
from PIL import Image
import requests
from dotenv import get_key

# Constants
API_URL = "https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0"
# API_KEY = get_key(".env", "HF_API_KEY")  # Make sure your .env file has HF_API_KEY=your_token
API_KEY = "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"  # Replace with your actual API key
HEADERS = {"Authorization": f"Bearer {API_KEY}"}
DATA_FOLDER = "Data"
IMAGE_FILE_PATH = r"Frontend/Files/ImageGeneration.data"

# Ensure the Data folder exists
os.makedirs(DATA_FOLDER, exist_ok=True)

def open_images(prompt):
    prompt_clean = prompt.replace(" ", "_")
    for i in range(1, 5):
        file_path = os.path.join(DATA_FOLDER, f"{prompt_clean}{i}.jpg")
        try:
            img = Image.open(file_path)
            print(f"Opening image: {file_path}")
            img.show()
            sleep(1)
        except IOError:
            print(f"[!] Could not open image: {file_path}")

async def query_api(payload):
    try:
        response = await asyncio.to_thread(requests.post, API_URL, headers=HEADERS, json=payload)
        response.raise_for_status()
        return response.content
    except requests.RequestException as e:
        print(f"[!] API Request failed: {e}")
        return None

async def generate_images(prompt: str):
    tasks = []
    for i in range(4):
        seed = randint(0, 1000000)
        input_prompt = f"{prompt}, quality=4k, sharpness=maximum, ultra high detail, high resolution, seed={seed}"
        payload = {"inputs": input_prompt}
        tasks.append(asyncio.create_task(query_api(payload)))

    responses = await asyncio.gather(*tasks)

    # for idx, content in enumerate(responses):
    #     if content:
    #         try:
    #             result = json.loads(content)
    #             if "generated_image" in result:
    #                 image_data = result["generated_image"]
    #             elif "image" in result:
    #                 image_data = result["image"]
    #             elif isinstance(result, dict) and isinstance(list(result.values())[0], str):
    #                 image_data = list(result.values())[0]
    #             else:
    #                 print(f"[!] Unexpected response format: {result}")
    #                 continue

    for idx, content in enumerate(responses):
        if content:
            try:
                image_filename = os.path.join(DATA_FOLDER, f"{prompt.replace(' ', '_')}{idx+1}.jpg")
                with open(image_filename, "wb") as f:
                    f.write(content)  # ✅ Directly write binary image
            except Exception as e:
                print(f"[!] Error saving image {idx+1}: {e}")
        else:
            print(f"[!] No response received for image {idx+1}")
            
                
# image_bytes = base64.b64decode(image_data)
#                 image_filename = os.path.join(DATA_FOLDER, f"{prompt.replace(' ', '_')}{idx+1}.jpg")
#                 with open(image_filename, "wb") as f:
#                     f.write(image_bytes)
#             except Exception as e:
#                 print(f"[!] Error saving image {idx+1}: {e}")
#         else:
#             print(f"[!] No response received for image {idx+1}")

def GenerateImages(prompt: str):
    asyncio.run(generate_images(prompt))
    open_images(prompt)

def main_loop():
    while True:
        try:
            if not os.path.exists(IMAGE_FILE_PATH):
                print("[!] Prompt file not found.")
                sleep(2)
                continue

            with open(IMAGE_FILE_PATH, "r") as f:
                data = f.read().strip()

            if not data:
                sleep(1)
                continue

            prompt, status = map(str.strip, data.split(","))
            if status.lower() == "true":
                print(f"[+] Generating images for prompt: '{prompt}'")
                GenerateImages(prompt)
                with open(IMAGE_FILE_PATH, "w") as f:
                    f.write("False, False")
                break
            else:
                sleep(1)

        except Exception as e:
            print(f"[!] Error in main loop: {e}")
            sleep(1)

if __name__ == "__main__":
    main_loop()
